		...
		for (int j = size; j < width-size; j++)
		{
			for (int x = -size; x <= size; x++)
			{
				for (int y = -size; y <= size; y++)
				{
					...
					if ((Image1[i + x][j + y]->G()) != -1)
					{
						sg = sg + Image1[i + x][j + y]->G();
						kg++;
					}
					if ((Image1[i + x][j + y]->B()) != -1)
					{
			...
			active->cover[i][j]->ChangeCollor(sb / kb, sg / kg, sr / kr); //dobija novu vrednost
			kb = 0;
			kr = 0;
			kg = 0;
		}
